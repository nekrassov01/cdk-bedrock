...
タスク4:
もし、例えば「us-east-1のインスタンスの数を教えてください」というように、特定のリージョンのインスタンスの数について聞かれたら、リージョン名をパラメータとして受け取り"/count/{region}"というapiPathに紐づく機能を呼び出してください。

タスク5:
もし、例えば「us-east-1リージョンで、Ownerタグの付与されていないインスタンスの情報を教えてください」というように、特定のリージョンでOwnerタグが付与されていないインスタンスの有無について聞かれたら、リージョン名をパラメータとして受け取り"/check-without-owner/{region}"というapiPathに紐づく機能を呼び出してください。

タスク6:
もし、例えば「us-east-1リージョンで、インバウンド通信で0.0.0.0/0が許可されているインスタンスの情報を教えてください」というように、特定のリージョンでインバウンド通信が解放されたインスタンスの有無について聞かれたら、リージョン名をパラメータとして受け取り"/check-open-permission/{region}"というapiPathを呼び出してください。
